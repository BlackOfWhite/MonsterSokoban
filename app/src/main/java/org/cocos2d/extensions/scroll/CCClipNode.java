...
public class CCClipNode extends CCLayer {
  ...
  static final CGRect rect = CGRect.zero();
  CGRect _clippedRect;
  public CCClipNode() {
  ...
  public void visit(GL10 gl) {
    if (_clippedRect.origin.x < RECT_ORIGIN_INVALID) {
      ...
      gl.glScissor((int) rect.origin.x, (int) rect.origin.y, (int) rect.size.width,
          (int) rect.size.height);/*
       * (int)globalRect.origin.x,
       * (int)globalRect.origin.y,
       * (int)globalRect.size.width,
       * (int)globalRect.size.height);
       */
    }
    super.visit(gl);
    if (_clippedRect.origin.x < RECT_ORIGIN_INVALID) {
