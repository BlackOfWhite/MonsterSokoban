...
public class CCTableView extends CCScrollView implements CCScrollViewDelegate {
  ...
  public final CCTableViewDelegate tDelegate;
  /**
   * index set to query the indexes of the cells used.
   */
  //NSMutableIndexSet* indices_;
  final HashSet<Integer> m_indices;
//    CCMutableArray<int*> *m_indices;
  /**
   * free list of cells
   */
  final ArrayList<CCTableViewCell> m_cellsFreed;
  /**
   * data source
   */
  ...
  public CCTableView(CGSize size, CCNode container) {
    ...
    m_cellsUsed = new ArrayList<>();
    m_cellsFreed = new ArrayList<>();
//        m_indices           = new CCMutableArray<int*>();
    m_indices = new HashSet<>();
    tDelegate = null;
...
//      }
  ...
  public void reloadData() {
    ...
    for (CCTableViewCell aM_cellsUsed : m_cellsUsed) {
      cell = aM_cellsUsed;
  ...
  private CCTableViewCell _cellWithIndex(int cellIndex) {
    ...
    for (Integer m_indice : m_indices) {
      if (cellIndex == m_indice) {
        for (CCTableViewCell aM_cellsUsed : m_cellsUsed) {
          if (aM_cellsUsed.getObjectID() == cellIndex) {
            found = aM_cellsUsed;
            break;
          }
        }
